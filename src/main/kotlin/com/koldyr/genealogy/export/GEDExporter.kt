...
class GEDExporter : Exporter {
    ...
    private fun families(person: Person, builder: StringBuilder) {
        if (person.family != null) {
            builder.append("1 ").append(OWN_FAMILY).append(" @F").append(person.family!!).append("@\n")
        }
        if (person.parentFamily != null) {
            builder.append("1 ").append(PARENT_FAMILY).append(" @F").append(person.parentFamily!!).append("@\n")
        }
    }
